...
bool IsCandleStrong(MqlRates &rates[])
{
    ...
    double bodySize  = MathAbs(rates[1].close - rates[1].open); // Gunakan bar[1] = bar entry (confirmed closed)
    ...
    if (!isStrong)
    {
        PrintFormat("%s [BODY RATIO] Ratio=%.2f%% < Min=%.2f%% → Candle LEMAH/INDECISION%s",
    ...
    {
        PrintFormat("✅ [BODY RATIO] Ratio=%.2f%% >= Min=%.2f%% → Candle KUAT, entry valid",
                    bodyRatio * 100, MinBodyRatio * 100);
    }
