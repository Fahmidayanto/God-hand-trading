                      ...
                      double lot_size, ulong magic, string tf)
{
    ...
    PrintFormat("💾 [SAVED] Trade #%d: %s %s | Entry: %.5f | Exit: %.5f | Profit: %.2f",
                g_TradeCount, type, tf, entry_price, exit_price, profit);
}
...
void ExportBacktestToCSV()
{
    ...
    FileWrite(handle, "Ticket", "Symbol", "Type", "EntryPrice", "ExitPrice", 
              "SL", "TP", "Profit", "EntryTime", "ExitTime", 
              "LotSize", "MagicNumber", "Timeframe");
...
void ExportAllData()
{
    ...
    // 5. Export market data M15 (5000 bars terakhir)
    ExportMarketDataToCSV(PERIOD_M15, 50000000);
    
    // 6. Export market data H1 (2000 bars terakhir)
...
input int DefaultTP_Buy_M15  = 2000;   // Take Profit untuk Buy Entry 1 (dalam poin) M15
input int DefaultSL_Sell_M15 = 3000;   // Stop Loss untuk Sell Entry 1 (dalam poin) M15
input int DefaultTP_Sell_M15 = 2000;   // Take Profit untuk Sell Entry 1 (dalam poin) M15
//--- Entry 2 parameters M15
input int Entry2SL_Buy_M15  = 3000;   // Stop Loss untuk Buy Entry 2 (dalam poin) M15
input int Entry2TP_Buy_M15  = 2500;   // Take Profit untuk Buy Entry 2 (dalam poin) M15
input int Entry2SL_Sell_M15 = 3000;  // Stop Loss untuk Sell Entry 2 (dalam poin) M15
...
void OnDeinit(const int reason)
{
    ...
    // Juga export dari history deals (untuk menangkap trade yang belum ter-record)
    ExportFromHistoryDeals();
}
...
//+------------------------------------------------------------------+
//| Export trade dari History Deals (fallback/tambahan)                |
//+------------------------------------------------------------------+
void ExportFromHistoryDeals()
{
    string dateStr = TimeToString(TimeCurrent(), TIME_DATE);
    StringReplace(dateStr, ".", "-");
    string filename = "History_Deals_" + _Symbol + "_" + dateStr + ".csv";
    ...
    if (totalDeals == 0)
    {
        Print("⚠️ [EXPORT] Tidak ada history deals.");
        return;
    }
    ...
    if (handle == INVALID_HANDLE)
    {
        PrintFormat("❌ [EXPORT] Gagal membuat file: %s", filename);
        return;
    }
    
    FileWrite(handle, "DealTicket", "OrderTicket", "PositionID", "Time", "Type", 
              "Direction", "Symbol", "Volume", "Price", "Profit", 
    ...
    for (int i = 0; i < totalDeals; i++)
    {
        ...
        string dirStr = "";
        if      (dealEntry == DEAL_ENTRY_IN)    dirStr = "IN";
        else if (dealEntry == DEAL_ENTRY_OUT)   dirStr = "OUT";
        else if (dealEntry == DEAL_ENTRY_INOUT) dirStr = "INOUT";
        ...
        FileWrite(handle,
                  ...
                  DoubleToString(HistoryDealGetDouble(dealTicket, DEAL_FEE), 2),
    ...
    FileClose(handle);
    PrintFormat("✅ [EXPORT] History deals berhasil di-export: %s (%d deals)", filename, exportedCount);
}
...
void DetectAndDraw_H1(MqlRates &rates_H1[], bool backfillMode)
{
    ...
    )
    {
        ...
        // --- ENTRY LOGIC ENABLED ---
        if (bosBullishConfirmedFlag_H1 && rates_H1[0].close > ema200_H1 && entryCountBuy_H1 < MaxEntriesPerCycle_H1) 
        {
            if (rates_H1[1].time > lastEntryTime_H1 + PeriodSeconds() * 3)
            {
                double entryPrice = rates_H1[1].close + 3 * _Point;
                double sl, tp;
                
                // Tentukan SL/TP berdasarkan entry ke berapa
                if (entryCountBuy_H1 == 0) {
                    // Entry 1 - Gunakan Default
                    sl = entryPrice - DefaultSL_Buy_H1 * _Point;
                    tp = entryPrice + DefaultTP_Buy_H1 * _Point;
                    Print("📌 [BoS Entry 1/3] SL: ", DefaultSL_Buy_H1, " poin, TP: ", DefaultTP_Buy_H1, " poin");
                } else if (entryCountBuy_H1 == 1) {
                    // Entry 2 - Gunakan Entry2SL/TP
                    sl = entryPrice - Entry2SL_Buy_H1 * _Point;
                    tp = entryPrice + Entry2TP_Buy_H1 * _Point;
                    Print("📌 [BoS Entry 2/3] SL: ", Entry2SL_Buy_H1, " poin, TP: ", Entry2TP_Buy_H1, " poin");
                } else {
                    // Entry 3 - Gunakan Entry3SL/TP
                    sl = entryPrice - Entry3SL_Buy_H1 * _Point;
                    tp = entryPrice + Entry3TP_Buy_H1 * _Point;
                    Print("📌 [BoS Entry 3/3] SL: ", Entry3SL_Buy_H1, " poin, TP: ", Entry3TP_Buy_H1, " poin");
                }
                
                if (lastClose_H4 > ema200_H4)
                {
                    if (trade.Buy(0.01, NULL, entryPrice, sl, tp, "H1_BoS Buy")) 
                    {
                        Print("✅ H1: ENTRY BUY di harga ", entryPrice);
                        Print("H1: [TRAILING RESET] Buy flags reset");
                        Print("📊 Entry Count (Buy): ", ++entryCountBuy_H1, "/", MaxEntriesPerCycle_H1);
                        lastEntryTime_H1 = TimeCurrent();
                        buyEntryPrice_H1 = entryPrice;
                        trailingActivatedBuy_H1 = false;
                        currentBuyTrade_H1.ticket = trade.ResultOrder();
                        currentBuyTrade_H1.entry_price = entryPrice;
                        currentBuyTrade_H1.sl = sl;
                        currentBuyTrade_H1.tp = tp;
                        currentBuyTrade_H1.entry_time = TimeCurrent();
                        currentBuyTrade_H1.type = "BUY";
                        PrintFormat("🚀 H1: [BUY ENTRY] Ticket: %I64u | Price: %.5f | SL: %.5f | TP: %.5f | Time: %s",
                        currentBuyTrade_H1.ticket,
                        currentBuyTrade_H1.entry_price,
                        currentBuyTrade_H1.sl,
                        currentBuyTrade_H1.tp,
                        TimeToString(currentBuyTrade_H1.entry_time, TIME_DATE|TIME_SECONDS));
                    } 
                    else 
                    {
                        Print("❌ H1: Gagal melakukan entry BUY");
                    }
                }
                else
                {
                    Print("H1: Entry Buy ditolak: H4 di bawah EMA200");
                }
            }
        }
        
        if (bosBullishConfirmedFlag_H1 && lastClose_H4 < ema200_H4)
        {
            Print("❌ H1: Tolak Buy: Kondisi bullish H1 tapi H4 bearish");
        }
    } 
    ...
    )
    {
        ...
        // ENTRY SELL SAAT BoS BEARISH H1
        // --- ENTRY LOGIC ENABLED ---
        if (bosBearishConfirmedFlag_H1 && rates_H1[0].close < ema200_H1 && entryCountSell_H1 < MaxEntriesPerCycle_H1)
        {
            if (rates_H1[1].time > lastSellEntryTime_H1 + PeriodSeconds() * 3)
            {
                double entryPrice = rates_H1[1].close - 3 * _Point;
                double sl, tp;
                
                // Tentukan SL/TP berdasarkan entry ke berapa
                if (entryCountSell_H1 == 0) {
                    // Entry 1 - Gunakan Default
                    sl = entryPrice + DefaultSL_Sell_H1 * _Point;
                    tp = entryPrice - DefaultTP_Sell_H1 * _Point;
                    Print("📌 [BoS Entry 1/3] SL: ", DefaultSL_Sell_H1, " poin, TP: ", DefaultTP_Sell_H1, " poin");
                } else if (entryCountSell_H1 == 1) {
                    // Entry 2 - Gunakan Entry2SL/TP
                    sl = entryPrice + Entry2SL_Sell_H1 * _Point;
                    tp = entryPrice - Entry2TP_Sell_H1 * _Point;
                    Print("📌 [BoS Entry 2/3] SL: ", Entry2SL_Sell_H1, " poin, TP: ", Entry2TP_Sell_H1, " poin");
                } else {
                    // Entry 3 - Gunakan Entry3SL/TP
                    sl = entryPrice + Entry3SL_Sell_H1 * _Point;
                    tp = entryPrice - Entry3TP_Sell_H1 * _Point;
                    Print("📌 [BoS Entry 3/3] SL: ", Entry3SL_Sell_H1, " poin, TP: ", Entry3TP_Sell_H1, " poin");
                }
                ...
                if (lastClose_H4 < ema200_H4)
                {
                    ...
                    {
                        Print("✅ H1: ENTRY SELL dilakukan di harga ", entryPrice);
                        Print("H1: [TRAILING RESET] Sell flags reset");
                        Print("📊 Entry Count (Sell): ", ++entryCountSell_H1, "/", MaxEntriesPerCycle_H1);
                        lastSellEntryTime_H1 = TimeCurrent();
                        sellEntryPrice_H1 = entryPrice;
                        trailingActivatedSell_H1 = false;
                        currentSellTrade_H1.ticket = trade.ResultOrder();
                        currentSellTrade_H1.entry_price = entryPrice;
                        currentSellTrade_H1.sl = sl;
                        currentSellTrade_H1.tp = tp;
                        currentSellTrade_H1.entry_time = TimeCurrent();
                        currentSellTrade_H1.type = "SELL";
                        PrintFormat("🚀 H1: [SELL ENTRY] Ticket: %I64u | Price: %.5f | SL: %.5f | TP: %.5f | Time: %s",
                        currentSellTrade_H1.ticket,
                        currentSellTrade_H1.entry_price,
                        currentSellTrade_H1.sl,
                        currentSellTrade_H1.tp,
                        TimeToString(currentSellTrade_H1.entry_time, TIME_DATE|TIME_SECONDS));
                    }
                    else
                    {
                        Print("❌ H1: Gagal melakukan entry SELL");
                    }
                }
                else
                {
                    Print("H1: Entry Sell ditolak: H4 di atas EMA200");
                }
            }
        }
        
        
        if (bosBearishConfirmedFlag_H1 && lastClose_H4 > ema200_H4)
        {
            Print("❌ H1: Tolak Sell: Kondisi bearish H1 tapi H4 bullish");
        }
    }
