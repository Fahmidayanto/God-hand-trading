...
TradeRecord_M15 activeBuyTrades_M15[3];   // Array untuk multiple buy entries M15
TradeRecord_M15 activeSellTrades_M15[3];  // Array untuk multiple sell entries M15
...
//+------------------------------------------------------------------+
//| Helper: Cari HH tertinggi antara dua waktu untuk CHoCH Bearish M15 |
//+------------------------------------------------------------------+
double FindHighestHHBetweenTimes_M15(MqlRates &rates[], datetime startTime, datetime endTime)
{
    double highestHH = -1;
    int foundCount = 0;
    
    PrintFormat("🔧 [FIND HH DEBUG] Scanning range: %s TO %s", TimeToString(startTime), TimeToString(endTime));
    PrintFormat("🔧 [ARRAY INFO] ArraySize: %d | rates[0].time=%s (newest), rates[size-1].time=%s (oldest)", 
                ArraySize(rates), TimeToString(rates[0].time), TimeToString(rates[ArraySize(rates)-1].time));
    
    // Loop untuk mencari HH tertinggi dalam range waktu [startTime, endTime]
    for(int i = 0; i < ArraySize(rates); i++)
    {
        // Jika bar berada dalam range [startTime, endTime]
        if(rates[i].time >= startTime && rates[i].time <= endTime)
        {
            foundCount++;
            PrintFormat("🔧 [HH FOUND] Bar #%d: time=%s, high=%.2f | Current highest=%.2f",
                       i, TimeToString(rates[i].time), rates[i].high, highestHH);
            
            if(rates[i].high > highestHH || highestHH == -1)
            {
                PrintFormat("  -> UPDATE: %.2f (old) -> %.2f (new)", highestHH, rates[i].high);
                highestHH = rates[i].high;
            }
        }
    }
    
    PrintFormat("✅ [FIND HH RESULT] Found %d bars in range | Highest HH: %.2f", foundCount, highestHH);
    return highestHH;
}
...
// ⭐ NEW: Check if H1 EMA200 is TRENDING UP (increasing)
bool IsH1TrendingUp()
{
    double ema_h1_current = 0, ema_h1_previous = 0;
    double emaBuffer[];
    
    // Get current bar H1 EMA
    if (CopyBuffer(iMA(_Symbol, PERIOD_H1, 200, 0, MODE_EMA, PRICE_CLOSE), 0, 0, 1, emaBuffer) > 0)
        ema_h1_current = emaBuffer[0];
    
    // Get previous bar H1 EMA
    if (CopyBuffer(iMA(_Symbol, PERIOD_H1, 200, 0, MODE_EMA, PRICE_CLOSE), 0, 1, 1, emaBuffer) > 0)
        ema_h1_previous = emaBuffer[0];
    
    bool h1_trending_up = (ema_h1_current > ema_h1_previous && ema_h1_current > 0);
    PrintFormat("📈 [H1 EMA SLOPE] Current: %.5f vs Previous: %.5f → Trending: %s", 
                ema_h1_current, ema_h1_previous, h1_trending_up ? "UP ✅" : "DOWN ❌");
    return h1_trending_up;
}

// ⭐ NEW: Check if H1 EMA200 is TRENDING DOWN (decreasing)
bool IsH1TrendingDown()
{
    double ema_h1_current = 0, ema_h1_previous = 0;
    double emaBuffer[];
    ...
    bool h1_trending_down = (ema_h1_current < ema_h1_previous && ema_h1_current > 0);
    PrintFormat("📉 [H1 EMA SLOPE] Current: %.5f vs Previous: %.5f → Trending: %s", 
                ema_h1_current, ema_h1_previous, h1_trending_down ? "DOWN ✅" : "UP ❌");
    return h1_trending_down;
}
...
void DetectAndDraw_M15(MqlRates &rates_M15[], bool backfillMode)
{
    ...
    if (!chochBullish_M15 && !isInTrendBullish_M15 &&
        (lastAcceptedHH_M15 > 0 && rates_M15[1].close > lastAcceptedHH_M15 + 50 * _Point))
        {
                ...
                // 🔧 FIX: Clear array entries untuk cycle baru
                for(int j = 0; j < 3; j++) {
                    activeBuyTrades_M15[j].ticket = 0;
                }
    ...
    if  (!chochBearish_M15 && !isInTrendBearish_M15 && // Hanya trigger jika belum dalam keadaan CHoCH/ tren bearish
        (lastAcceptedLL_M15 > 0 && rates_M15[1].close < lastAcceptedLL_M15 - 5 * _Point)) // Cek apakah close menembus LL terakhir (dengan sedikit buffer)
        {
            ...
            // === PERBAIKAN NEW: Cari HH tertinggi antara lastAcceptedLL dan current bar ===
            // Cari HH yang terbentuk SETELAH LL diterima dan SEBELUM CHoCH Bearish trigger
            double highestHHAfterLL_M15 = FindHighestHHBetweenTimes_M15(rates_M15, lastLoggedValidLLTime_M15, rates_M15[1].time);
            
            // Jika ada HH dalam range tersebut, gunakan itu sebagai lastAcceptedHH baru (REGARDLESS of whether it's higher or lower)
            if(highestHHAfterLL_M15 > -1)
            {
                lastAcceptedHH_M15 = highestHHAfterLL_M15;
                PrintFormat("🔍 [CHoCH BEARISH - HH SCAN M15] HH tertinggi ditemukan antara LL (%.2f @ %s) dan sekarang: %.2f (old preChochHH: %.2f)",
                            tempLL_M15, TimeToString(lastLoggedValidLLTime_M15), highestHHAfterLL_M15, preChochHH_M15);
            }
            else
            {
                // Jika tidak ada HH ditemukan di range itu, gunakan HH lama sebagai fallback
                lastAcceptedHH_M15 = lastLoggedValidHH_M15;
                PrintFormat("📌 [CHoCH BEARISH - NO HH FOUND IN RANGE M15] Tidak ditemukan HH dalam range (LL: %.2f @ %s sampai %s). Fallback ke lastLoggedValidHH: %.2f",
                            tempLL_M15, TimeToString(lastLoggedValidLLTime_M15), TimeToString(rates_M15[1].time), lastAcceptedHH_M15);
            }
            ...
            // 🔧 FIX: Clear array entries untuk cycle baru
            for(int j = 0; j < 3; j++) {
                activeSellTrades_M15[j].ticket = 0;
            }
    ...
    ))
    {
        ...
        // bool isBlackoutHour = (currentHour == 2 || currentHour == 14 || currentHour == 22);  // ⛔ DISABLED - Nonaktifkan blackout hour filter
        
        // 🔍 DEBUG: Check all entry conditions with proper candle indexing
        if (bosBullishConfirmedFlag_M15) {
            bool cond1 = bosBullishConfirmedFlag_M15;
            bool cond2 = rates_M15[1].close > ema200_M15;  // FIXED: Use completed candle [1] instead of [0]
            bool cond3 = entryCountBuy_M15 < MaxEntriesPerCycle_M15;
            bool cond4 = IsH1ConfirmedBullish();
            bool cond5 = IsEMATrendingUp_M15();
            bool cond6_h1trend = IsH1TrendingUp();  // ⭐ NEW: H1 EMA trending UP
            // bool cond7 = !isBlackoutHour;  // ⛔ DISABLED - Nonaktifkan blackout hour check
            bool cond7 = true;  // ✓ ALWAYS TRUE - Nonaktifkan blackout hour filter
            PrintFormat("🔍 [ENTRY CONDITIONS BUY M15] BoS:%s | M15Close(%.5f)>EMA200(%.5f):%s | EntryCount(%d)<%d:%s | H1Bull:%s | H1TrendUp:%s | M15TrendUp:%s | NoBlackout(hr=%d):%s",
                cond1 ? "✓" : "✗", rates_M15[1].close, ema200_M15, cond2 ? "✓" : "✗", entryCountBuy_M15, MaxEntriesPerCycle_M15, cond3 ? "✓" : "✗",
                cond4 ? "✓" : "✗", cond6_h1trend ? "✓" : "✗", cond5 ? "✓" : "✗", currentHour, cond7 ? "✓" : "✗");
        }
        
        // ⭐ UPDATED: Added H1 trend confirmation check - Only BUY if H1 EMA is TRENDING UP
        if (bosBullishConfirmedFlag_M15 && rates_M15[1].close > ema200_M15 && entryCountBuy_M15 < MaxEntriesPerCycle_M15 && IsH1ConfirmedBullish() && IsH1TrendingUp() && IsEMATrendingUp_M15()) 
        {
            ...
            double entryPrice_M15 = rates_M15[1].close + 3 * _Point; // Sedikit di atas close
                ...
                // ⚙️ SL statis menggunakan input user DefaultSL_Buy_M15
                sl_M15 = entryPrice_M15 - DefaultSL_Buy_M15 * _Point;  // SL statis dari input
                
                if (entryCountBuy_M15 == 0) {
                    // Entry 1 - Gunakan Default TP
                    tp_M15 = entryPrice_M15 + DefaultTP_Buy_M15 * _Point;
                    Print("📌 [BoS Entry 1/3] SL: ", sl_M15, " (Static SL), TP: ", DefaultTP_Buy_M15, " poin");
                } else if (entryCountBuy_M15 == 1) {
                    // Entry 2 - Gunakan Entry2TP
                    tp_M15 = entryPrice_M15 + Entry2TP_Buy_M15 * _Point;
                    Print("📌 [BoS Entry 2/3] SL: ", sl_M15, " (Static SL), TP: ", Entry2TP_Buy_M15, " poin");
                } else {
                    // Entry 3 - Gunakan Entry3TP
                    tp_M15 = entryPrice_M15 + Entry3TP_Buy_M15 * _Point;
                    Print("📌 [BoS Entry 3/3] SL: ", sl_M15, " (Static SL), TP: ", Entry3TP_Buy_M15, " poin");
                }
                ...
                if (trade.Buy(0.05, NULL, entryPrice_M15, sl_M15, tp_M15, "BoS Buy M15")) 
                    ...
                    // 🔧 FIX: Simpan ke array agar entry ke-2,3 tidak overwrite entry ke-1
                    int buyTradeIndex = entryCountBuy_M15 - 1;
    ...
    )
    {
        ...
        // bool isSellBlackoutHour = (sellHour == 2 || sellHour == 14 || sellHour == 22);  // ⛔ DISABLED - Nonaktifkan blackout hour filter
        bool isHighLiquidityHourSell = (sellHour >= 6 && sellHour <= 11); // SELL only during 06-11 UTC
        
        // 🔍 DEBUG: Check all entry conditions for SELL
        if (bosBearishConfirmedFlag_M15) {
            bool cond1 = bosBearishConfirmedFlag_M15;
            bool cond2 = rates_M15[1].close < ema200_M15;  // FIXED: Use completed candle [1]
            bool cond3 = entryCountSell_M15 < MaxEntriesPerCycle_M15;
            bool cond4 = IsH1ConfirmedBearish();
            bool cond5 = !IsEMATrendingUp_M15();
            bool cond6_h1trend = IsH1TrendingDown();  // ⭐ NEW: H1 EMA trending DOWN
            // bool cond7 = !isSellBlackoutHour;  // ⛔ DISABLED - Nonaktifkan blackout hour check
            bool cond7 = true;  // ✓ ALWAYS TRUE - Nonaktifkan blackout hour filter
            bool cond8 = isHighLiquidityHourSell;
            PrintFormat("🔍 [ENTRY CONDITIONS SELL M15] BoS:%s | M15Close(%.5f)<EMA200(%.5f):%s | EntryCount(%d)<%d:%s | H1Bear:%s | H1TrendDn:%s | M15TrendDn:%s | NoBlackout(hr=%d):%s | LiquidHr:%s",
                cond1 ? "✓" : "✗", rates_M15[1].close, ema200_M15, cond2 ? "✓" : "✗", entryCountSell_M15, MaxEntriesPerCycle_M15, cond3 ? "✓" : "✗",
                cond4 ? "✓" : "✗", cond6_h1trend ? "✓" : "✗", cond5 ? "✓" : "✗", sellHour, cond7 ? "✓" : "✗", cond8 ? "✓" : "✗");
        }
        
        // ⭐ UPDATED: Added H1 trend confirmation check - Only SELL if H1 EMA is TRENDING DOWN
        if (bosBearishConfirmedFlag_M15 && rates_M15[1].close < ema200_M15 && entryCountSell_M15 < MaxEntriesPerCycle_M15 && IsH1ConfirmedBearish() && IsH1TrendingDown() && !IsEMATrendingUp_M15() && isHighLiquidityHourSell)
        {
            // ✅ REMOVED: Time gap requirement - Entry 2/3 dapat trigger immediate
            double entryPrice_M15 = rates_M15[1].close - 3 * _Point;  // Sedikit di bawah close
                ...
                // Tentukan SL/TP berdasarkan entry ke berapa
                // ⚙️ SL statis menggunakan input user DefaultSL_Sell_M15
                sl_M15 = entryPrice_M15 + DefaultSL_Sell_M15 * _Point;  // SL statis dari input
                
                if (entryCountSell_M15 == 0) {
                    // Entry 1 - Gunakan Default TP
                    tp_M15 = entryPrice_M15 - DefaultTP_Sell_M15 * _Point;
                    Print("📌 [BoS Entry 1/3] SL: ", sl_M15, " (Static SL), TP: ", DefaultTP_Sell_M15, " poin");
                } else if (entryCountSell_M15 == 1) {
                    // Entry 2 - Gunakan Entry2TP
                    tp_M15 = entryPrice_M15 - Entry2TP_Sell_M15 * _Point;
                    Print("📌 [BoS Entry 2/3] SL: ", sl_M15, " (Static SL), TP: ", Entry2TP_Sell_M15, " poin");
                } else {
                    // Entry 3 - Gunakan Entry3TP
                    tp_M15 = entryPrice_M15 - Entry3TP_Sell_M15 * _Point;
                    Print("📌 [BoS Entry 3/3] SL: ", sl_M15, " (Static SL), TP: ", Entry3TP_Sell_M15, " poin");
                }
                ...
                if (trade.Sell(0.05, NULL, entryPrice_M15, sl_M15, tp_M15))
                    {
                        ...
                        // 🔧 FIX: Simpan ke array agar entry ke-2,3 tidak overwrite entry ke-1
                        int sellTradeIndex = entryCountSell_M15 - 1;
...
void OnTick()
{
    ...
    // 🔥 BUG FIX: Dihapus agar M15 tetap dieksekusi setiap candle tutup (xx:15, xx:30, xx:45)
    bool isNewH1 = false;
    if (rates_H1[0].time != lastBarTime_H1) {
    ...
    // 🔧 FIX: Check closure untuk single struct (backward compatibility)
    CheckTradeClosure_M15(currentBuyTrade_M15);
    CheckTradeClosure_M15(currentSellTrade_M15);
    
    // 🔧 FIX: Loop melalui array untuk check ALL active entries (entry 1, 2, 3)
    for(int i = 0; i < activeBuyCount_M15; i++)
    {
    ...
    for(int i = 0; i < activeSellCount_M15; i++)
    {
    ...
    //+------------------------------------------------------------------+
    //| LIVE EXPORT DATA FOR AI ENGINE                                   |
    //| Dipanggil otomatis pada setiap selesai bar M15 untuk Telegram    |
    //+------------------------------------------------------------------+
    if(!MQLInfoInteger(MQL_TESTER)) // Hanya jalan di Live Chart, bukan di Tester
    {
        Print("📡 [LIVE SYNC] Menyimpan snapshot CSV terbaru untuk AI Trading Engine...");
        ...
        // Membatasi copy data market agar tidak berat di RAM (cukup secukupnya untuk fitur AI)
        ExportMarketDataToCSV(PERIOD_M15, 2000); 
        ExportMarketDataToCSV(PERIOD_H1, 1000);
    }
}