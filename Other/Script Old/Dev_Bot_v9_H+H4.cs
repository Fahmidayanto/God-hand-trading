...
void CaptureOpenTrades()
{
    ...
    for (int i = 0; i < openPositions; i++)
    {
        ...
        SaveTradeToArray(ticket, _Symbol, type,
                        ...
                        0,0, 0, GetCurrentSession());  // Assume no spread/commission for open trades at capture time
    }
}
        ...
        if(found)
        {
            ...
            for(int j = totalDeals-1; j >= 0; j--)
            {
                ...
                if(HistoryDealGetInteger(dealTicket, DEAL_POSITION_ID) == tradeRec.ticket)
                {
                    ...
                    // Akumulasi commission & swap (signed values dari MT5)
...
//+------------------------------------------------------------------+
// BUY: harga H4 di ATAS EMA H4 → trend bullish → izinkan BUY
// SELL: harga H4 di BAWAH EMA H4 → trend bearish → izinkan SELL
// Jika harga H4 masih di ATAS EMA → JANGAN SELL (trend masih bullish)
// Jika harga H4 masih di BAWAH EMA → JANGAN BUY (trend masih bearish)
...
bool IsH4ConfirmedBullish()
{
    ...
    bool bullishH4 = (h4Rate[0].close > emaH4);

    if (bullishH4)
        PrintFormat("✅ [H4 EMA FILTER] BUY OK: H4 Close (%.5f) > EMA%d H4 (%.5f) — Trend H4 Bullish",
                    h4Rate[0].close, EMA_H4_Period, emaH4);
    else
        PrintFormat("%s [H4 EMA FILTER] BUY DITOLAK: H4 Close (%.5f) <= EMA%d H4 (%.5f) — Trend H4 masih Bearish%s",
                    H4EMA_LogOnly ? "📊 [LOG ONLY]" : "❌",
                    h4Rate[0].close, EMA_H4_Period, emaH4,
...
bool IsH4ConfirmedBearish()
{
    ...
    bool bearishH4 = (h4Rate[0].close < emaH4);

    if (bearishH4)
        PrintFormat("✅ [H4 EMA FILTER] SELL OK: H4 Close (%.5f) < EMA%d H4 (%.5f) — Trend H4 Bearish",
                    h4Rate[0].close, EMA_H4_Period, emaH4);
    else
        PrintFormat("%s [H4 EMA FILTER] SELL DITOLAK: H4 Close (%.5f) >= EMA%d H4 (%.5f) — Trend H4 masih Bullish%s",
                    H4EMA_LogOnly ? "📊 [LOG ONLY]" : "❌",
                    h4Rate[0].close, EMA_H4_Period, emaH4,
                    H4EMA_LogOnly ? " (entry tetap diizinkan)" : " (entry ditolak)");
...
void DetectAndDraw_M15(MqlRates &rates_M15[], bool backfillMode)
{
    ...
    ))
    {
        ...
        // --- ENTRY LOGIC ENABLED WITH FILTERS (Sequential Rejection Logic) ---
        if (bosBullishConfirmedFlag_M15 &&
    ...
    )
    {
        ...
        // --- ENTRY LOGIC WITH H1 & EMA FILTERS (Sequential Rejection Logic) ---
        if (bosBearishConfirmedFlag_M15 &&
            rates_M15[0].close < ema200_M15 &&
