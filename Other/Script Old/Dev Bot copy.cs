...
input int DefaultTP_Buy  = 1500;   // Take Profit untuk Buy (dalam poin)
input int DefaultSL_Sell = 1000;   // Stop Loss untuk Sell (dalam poin)
input int DefaultTP_Sell = 1500;   // Take Profit untuk Sell (dalam poin)
...
void ApplyTrailingStop()
{
    ...
    for(int i = PositionsTotal()-1; i >= 0; i--)
    {
        ...
        if(posType == POSITION_TYPE_BUY)
        {
            ...
            else if(profitPoints >= 2500 && trailingActivatedBuy == true && sl <= positionOpen + DefaultSL_Buy * _Point)
            {
...
void OnTick()
{
    ...
    for (int i = ArraySize(rates) - WindowSize - 1; i >= WindowSize; i--)
    {
        ...
        if (isLow)
        {
            ...
            if (lastAcceptedHH > -1 && !chochBullish && !hhAfterChochConfirmedFlag)
                {
                    if (lastAcceptedLL == -1 || rates[i].low > lastAcceptedLL)
                    {
                        lastAcceptedLL = rates[i].low;
                        PrintFormat("✅ [MODE 1B] Update Last Accepted LL (HL setelah HH): %.2f | Time: %s",
                                    lastAcceptedLL, TimeToString(rates[i].time));
                        UpdateAcceptedLevelVisuals(lastAcceptedLL, "LL", rates[i].time);
                    }
                }
