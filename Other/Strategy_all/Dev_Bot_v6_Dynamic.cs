    ...
    if (!PositionSelectByTicket(tradeRec.ticket))
    {
        ...
        for(int i = totalDeals-1; i >= 0; i--)
        {
            ...
            if(HistoryDealGetInteger(dealTicket, DEAL_POSITION_ID) == tradeRec.ticket)
            {
                ...
                found = true;
                PrintFormat("   ✅ Found deal for Ticket %I64u: Exit Price=%.5f, Profit=%.2f", 
                            tradeRec.ticket, exitPrice, profit);
                break;
            }
        }
        ...
        if(found)
        {
            ...
            // Hitung spread cost, commission dan swap dari history deals
            double spreadCost = 0;
            double commission = 0;
            double swapVal = 0;
            HistorySelect(0, TimeCurrent());
...
bool IsH1ConfirmedBearish()
{
    ...
    if (!bearishConfirmed)
    {
        ...
        RecordRejectedToBacktestTrades("SELL", estimatedEntry, "H1 EMA200 Filter");
    }
...
void DetectAndDraw_M15(MqlRates &rates_M15[], bool backfillMode)
{
    ...
    ))
    {
        ...
        ) 
        {
            ...
            double entryPrice_M15 = rates_M15[1].close + 3 * _Point; // Sedikit di atas close
                ...
                // ✅ SL untuk semua entries berada di last accepted LL (market structure)
                sl_M15 = lastAcceptedLL_M15;  // SL di last accepted LL
                
                if (entryCountBuy_M15 == 0) {
                    // Entry 1 - Gunakan Default TP
                    tp_M15 = entryPrice_M15 + DefaultTP_Buy_M15 * _Point;
                    Print("📌 [BoS Entry 1/3] SL: ", lastAcceptedLL_M15, " (lastAcceptedLL), TP: ", DefaultTP_Buy_M15, " poin");
                } else if (entryCountBuy_M15 == 1) {
                    // Entry 2 - Gunakan Entry2TP
                    tp_M15 = entryPrice_M15 + Entry2TP_Buy_M15 * _Point;
                    Print("📌 [BoS Entry 2/3] SL: ", lastAcceptedLL_M15, " (lastAcceptedLL), TP: ", Entry2TP_Buy_M15, " poin");
                } else {
                    // Entry 3 - Gunakan Entry3TP
                    tp_M15 = entryPrice_M15 + Entry3TP_Buy_M15 * _Point;
                    Print("📌 [BoS Entry 3/3] SL: ", lastAcceptedLL_M15, " (lastAcceptedLL), TP: ", Entry3TP_Buy_M15, " poin");
                }
    ...
    )
    {
        ...
        )
        {
            ...
            // Tentukan SL/TP berdasarkan entry ke berapa
            // ✅ SL untuk semua entries berada di last accepted HH (market structure)
            sl_M15 = lastAcceptedHH_M15;  // SL di last accepted HH
            
            if (entryCountSell_M15 == 0) {
                // Entry 1 - Gunakan Default TP
                tp_M15 = entryPrice_M15 - DefaultTP_Sell_M15 * _Point;
                Print("📌 [BoS Entry 1/3] SL: ", lastAcceptedHH_M15, " (lastAcceptedHH), TP: ", DefaultTP_Sell_M15, " poin");
            } else if (entryCountSell_M15 == 1) {
                // Entry 2 - Gunakan Entry2TP
                tp_M15 = entryPrice_M15 - Entry2TP_Sell_M15 * _Point;
                Print("📌 [BoS Entry 2/3] SL: ", lastAcceptedHH_M15, " (lastAcceptedHH), TP: ", Entry2TP_Sell_M15, " poin");
            } else {
                // Entry 3 - Gunakan Entry3TP
                tp_M15 = entryPrice_M15 - Entry3TP_Sell_M15 * _Point;
                Print("📌 [BoS Entry 3/3] SL: ", lastAcceptedHH_M15, " (lastAcceptedHH), TP: ", Entry3TP_Sell_M15, " poin");
            }
...
{
    //+------------------------------------------------------------------+
    //| SESSION ZONE TRACKING & VISUALS - Dipanggil setiap tick          |
    //+------------------------------------------------------------------+
    ...
    if(Period() == PERIOD_M15)
    {
        ...
        // 🔧 FIX: Loop melalui SEMUA slots array (bukan counter) - selalu cek 3 entries
        PrintFormat("🔍 [ARRAY CHECK] Checking %d BUY trades in array", activeBuyCount_M15);
        for(int i = 0; i < 10; i++)
        {
            if(activeBuyTrades_M15[i].ticket > 0)
            {
                PrintFormat("   Slot [%d]: Ticket=%I64u", i, activeBuyTrades_M15[i].ticket);
                CheckTradeClosure_M15(activeBuyTrades_M15[i]);
            }
        }
        
        PrintFormat("🔍 [ARRAY CHECK] Checking %d SELL trades in array", activeSellCount_M15);
        for(int i = 0; i < 10; i++)
        {
            if(activeSellTrades_M15[i].ticket > 0)
            {
                PrintFormat("   Slot [%d]: Ticket=%I64u", i, activeSellTrades_M15[i].ticket);
