...
class Tracker {
  ...
  private async init() {
    try {
      // Only initialize Tauri features if running in Tauri
      if (!isTauri()) {
        // Running in browser, use fallback values
        this.params = {
          client_id: "web-" + Math.random().toString(36).substring(7),
      ...
      // Silent fallback for tracking initialization
      this.params = {
        client_id: "unknown",
        os: navigator.userAgent,
      };
