...
for col in v11_cols:
    if col in ea_executed.columns:
        model_df[col] = ea_executed[col].astype(float)
...
# Predictions v11 (Planned R:R + Reject Aware)
feat_v11_mfe = scaler_v11_mfe.feature_names_in_
feat_v11_mae = scaler_v11_mae.feature_names_in_
pred_v11_mfe = mfe_v11.predict(scaler_v11_mfe.transform(model_df[feat_v11_mfe])) * price_ratio.values
pred_v11_mae = mae_v11.predict(scaler_v11_mae.transform(model_df[feat_v11_mae])) * price_ratio.values
rr_v11 = pred_v11_mfe / np.maximum(1.0, pred_v11_mae)
...
ea_executed['pass_v11'] = (rr_v11 >= 1.05) & (ea_executed['is_news_blackout'] == 0) & (~is_weekend_gap_veto)
...
ea_executed['lot_v11'] = [get_lot(r) if p else 0.0 for r, p in zip(rr_v11, ea_executed['pass_v11'])]
ea_executed['dyn_pnl_v11'] = ea_executed['net_profit'] * (ea_executed['lot_v11'] / 0.01)

print("\n=== PERBANDINGAN PER TAHUN: EA vs MODEL v9 vs MODEL v10 vs MODEL v11 ===")
...
tot_v11_trd, tot_v11_pnl, tot_v11_dyn, tot_v11_wins = 0, 0, 0, 0
...
for yr in years:
    ...
    # v11
    sub_v11 = sub[sub['pass_v11']]
    v11_trd = len(sub_v11)
    v11_pnl = sub_v11['net_profit'].sum()
    v11_dyn = sub_v11['dyn_pnl_v11'].sum()
    v11_wr = (sub_v11['net_profit'] > 0).mean() * 100 if v11_trd > 0 else 0
    ...
    tot_v11_pnl += v11_pnl
    tot_v11_dyn += v11_dyn
    tot_v11_wins += (sub_v11['net_profit'] > 0).sum()
    
    print(f"{yr} | EA: {ea_pnl:+8.1f} USD ({ea_wr:4.1f}%) | v9: {v9_pnl:+8.1f} USD ({v9_wr:4.1f}%) | v10: {v10_pnl:+8.1f} USD ({v10_wr:4.1f}%) | v11 Flat: {v11_trd:3d} trd, {v11_pnl:+8.1f} USD ({v11_wr:4.1f}%) | v11 Dyn: {v11_dyn:+10.1f} USD")

print(f"TOTAL | EA: {tot_ea_pnl:+8.1f} USD ({(tot_ea_wins/tot_ea_trd)*100:4.1f}%) | v9: {tot_v9_pnl:+8.1f} USD ({(tot_v9_wins/tot_v9_trd)*100:4.1f}%) | v10: {tot_v10_pnl:+8.1f} USD ({(tot_v10_wins/tot_v10_trd)*100:4.1f}%) | v11 Flat: {tot_v11_trd:3d} trd, {tot_v11_pnl:+8.1f} USD ({(tot_v11_wins/tot_v11_trd)*100:4.1f}%) | v11 Dyn: {tot_v11_dyn:+10.1f} USD")
