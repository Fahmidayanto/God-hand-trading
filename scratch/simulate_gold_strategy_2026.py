...
ENTRY2_SL_BUY = 2000    # 200 Pips
ENTRY2_TP_BUY = 2500    # 250 Pips
ENTRY2_SL_SELL = 2000   # 200 Pips
ENTRY2_TP_SELL = 2500   # 250 Pips

# Dynamic SL Limits
MIN_SL_DISTANCE = 1500  # 150 Pips
MAX_SL_DISTANCE = 3000  # 300 Pips
BUFFER_POINTS = 1000    # 100 Pips

# Path Files 2026
DATA_DIR = r"b:\Project MT5\Backtest_result"
M15_FILE = os.path.join(DATA_DIR, "MarketData_XAUUSD_M15_2026-08-12.csv")
H1_FILE = os.path.join(DATA_DIR, "MarketData_XAUUSD_H1_2026-08-12.csv")
H4_FILE = os.path.join(DATA_DIR, "MarketData_XAUUSD_H4_2026-08-12.csv")
LLHHBOS_FILE = os.path.join(DATA_DIR, "LLHHBOSData_XAUUSD_2026-08-12.csv")
SESSION_FILE = os.path.join(DATA_DIR, "SessionZone_XAUUSD_2026-08-12.csv")

OUTPUT_CSV = r"b:\Project MT5\scratch\Backtest_Results_XAUUSD_2026_CHoCH_BoS_Simulated_v3.csv"

# ==========================================
# DATA LOADING
# ==========================================
print("[INFO] Loading 2026 Market Data...")
...
# Load LLHHBOS Data
bos_events = []
if os.path.exists(LLHHBOS_FILE):
    with open(LLHHBOS_FILE, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        for row in reader:
            if not row or len(row) < 5:
                continue
            if row[0] in ['Type', '=== Market Structure Events (Sorted by Time) ===']:
                continue
            ...
            tf = row[4].strip()
            if time_str and tf == 'M15':
                try:
                    dt = datetime.strptime(time_str, '%Y.%m.%d %H:%M:%S')
                    bos_events.append({'type': event_type, 'direction': direction, 'price': price, 'time': dt})
                except Exception:
                    pass

print(f"[SUCCESS] Loaded M15 Bars: {len(m15_df)}, H1 Bars: {len(h1_df)}, H4 Bars: {len(h4_df)}, Events: {len(bos_events)}")

# Load Session Zone Data
sessions = []
if os.path.exists(SESSION_FILE):
    sess_df = pd.read_csv(SESSION_FILE)
    sess_df['StartTime'] = pd.to_datetime(sess_df['StartTime'], format='%Y.%m.%d %H:%M:%S')
    sess_df['EndTime'] = pd.to_datetime(sess_df['EndTime'], format='%Y.%m.%d %H:%M:%S')
    sessions = sess_df.to_dict('records')

def get_session_name(bar_time):
    for s in sessions:
        if s['StartTime'] <= bar_time <= s['EndTime']:
            return s['Session']
...
last_accepted_hh = -1.0
last_accepted_ll = -1.0
pre_choch_hh = -1.0
pre_choch_ll = -1.0
...
def is_h1_bullish(bar_time, close_price):
    h1_bars = h1_df[h1_df.index <= bar_time]
    if len(h1_bars) == 0:
        return True
...
def is_h4_bullish(bar_time, close_price):
    h4_bars = h4_df[h4_df.index <= bar_time]
    if len(h4_bars) == 0:
        return True
...
# ==========================================
print("[INFO] Running M15 Strategy Backtest Simulation...")
